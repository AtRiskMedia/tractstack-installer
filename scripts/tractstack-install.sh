...
if [ "$USER" == "$INSTALL_USER" ]; then
	...
	sudo -H -u "$INSTALL_USER" bash -c '~/t8k/"$TARGET"/scripts/tractstack-home-init.sh'
	sudo -H -u "$INSTALL_USER" bash -c '~/t8k/"$TARGET"/scripts/tractstack-init-drupal.sh '"$DB_PASS"' '"$DRUPAL_PASS"
	cp ../files/drupal/web.config /home/t8k/"$TARGET"/"$NAME"/srv/public_html/drupal/oauth_keys
	./fix-drupal.sh /home/t8k/"$TARGET"/"$NAME"/srv/public_html/drupal/web "$NAME"
...
if [ "$USER" == "$INSTALL_USER" ]; then
	...
	cp /home/t8k/"$TARGET"/"$NAME"/src/gatsby-starter-storykeep/src/custom/codehooks.tsx.example /home/"$NAME"/src/gatsby-starter-storykeep/src/custom/codehooks.tsx
	...
	cp /home/t8k/"$TARGET"/"$NAME"/src/gatsby-starter-tractstack/src/custom/codehooks.tsx.example /home/"$NAME"/src/gatsby-starter-tractstack/src/custom/codehooks.tsx
	...
	echo ""
	echo Running build on storykeep
	sudo -H -u "$INSTALL_NAME" bash -c '~/t8k/'"$TARGET"'/scripts/build.sh back'
fi
